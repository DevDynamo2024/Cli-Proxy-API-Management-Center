    ...
    <div className={styles.container}>
      ...
      <div className={styles.grid}>
        <div className={styles.leftColumn}>
          ...
          <Card className={styles.panel}>
            <div className={styles.row}>
              <div>{t('api_key_policies.routing_title', { defaultValue: '模型路由（按时间窗口比例）' })}</div>
              <div className={styles.ruleActions}>
                <Button
                  variant="secondary"
                  disabled={disableControls || !selectedKey}
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            updateRoutingRule(idx, {
                              stickyWindowSeconds: clampInt(e.target.value, 1, 3600 * 24 * 30, DEFAULT_STICKY_WINDOW_SECONDS)
                            })
                          }
                        >
                          <option value="1800">{t('api_key_policies.routing_window_30m', { defaultValue: '30 分钟' })}</option>
                          <option value="3600">{t('api_key_policies.routing_window_1h', { defaultValue: '1 小时' })}</option>
                          <option value="7200">{t('api_key_policies.routing_window_2h', { defaultValue: '2 小时' })}</option>
                          <option value="14400">{t('api_key_policies.routing_window_4h', { defaultValue: '4 小时' })}</option>
                          <option value="28800">{t('api_key_policies.routing_window_8h', { defaultValue: '8 小时' })}</option>
                          <option value="86400">{t('api_key_policies.routing_window_24h', { defaultValue: '24 小时' })}</option>
                        </select>
                      </div>
                    </div>
                  </div>
